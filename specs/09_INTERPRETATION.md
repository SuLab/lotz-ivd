# Module 09: Biological Interpretation
...
- DE results from Module 08: `results/differential/`
- Integrated and annotated AnnData objects from Module 07 (with `obs['cell_type']` from de novo annotation)
...
### Notebook: `notebooks/09_interpretation.ipynb`
...
4. Cluster redundant terms using semantic similarity via `goatools` (Python) to reduce noise
...
- Use the standard cisTarget feather files from the pySCENIC resources (hg38, 10kb upstream/downstream): `hg38_10kbp_up_10kbp_down_full_tx_v10_clust.genes_vs_motifs.rankings.feather` and the corresponding motif-to-TF annotation file.
- Custom IVD-relevant gene sets (listed in Part 1 and Part 3) should be defined inline in the script — they are small curated lists that don't warrant a separate file format.
