#!/usr/bin/env python3
"""Module 05 — Workflow B: scANVI integration for IVD atlas.
...
Usage:
    python3 scripts/05b_integration_scanvi.py                  # All objects
    python3 scripts/05b_integration_scanvi.py --object NP      # Single object
    python3 scripts/05b_integration_scanvi.py --validate-only  # Validation only
    python3 scripts/05b_integration_scanvi.py --force          # Re-run even if outputs exist
"""
...
INT_DIR = BASE / "data" / "integrated" / "scanvi"
...
def _plot_object_umaps(adata, object_name):
    ...
    plt.tight_layout()
    fig.savefig(RESULTS_DIR / f"umap_scanvi_{object_name}.png",
                dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved: umap_scanvi_{object_name}.png")
...
def validate_integration():
    """Validate scANVI integration outputs. Returns (pass, messages)."""
    print("\n" + "=" * 60)
    print("Validating scANVI integration outputs")
    print("=" * 60)
    ...
    for obj_name in ["NP", "AF", "CEP", "all_cells"]:
        ...
        if path.exists():
            messages.append(f"PASS: scanvi/{obj_name} output exists")
            adata = sc.read_h5ad(path, backed='r')
            ...
            if 'X_umap' in adata.obsm:
                try:
                    ...
                    if n_clusters > 1:
                        messages.append(f"PASS: scanvi/{obj_name} has {n_clusters} clusters at res=0.5 (no blob)")
                    else:
                        messages.append(f"WARNING: scanvi/{obj_name} has only {n_clusters} cluster at res=0.5 (blob?)")
                    del adata_full
                except Exception as e:
                    messages.append(f"WARNING: scanvi/{obj_name} blob check failed: {e}")
            else:
                messages.append(f"WARNING: scanvi/{obj_name} missing X_umap")

            adata.file.close()
        else:
            messages.append(f"FAIL: scanvi/{obj_name} output missing")
            all_pass = False
    ...
    print(f"\n{'='*60}")
    print(f"scANVI VALIDATION: {'ALL CHECKS PASSED' if all_pass else 'SOME CHECKS FAILED'}")
    print(f"{'='*60}")
...
def main():
    ...
    print("=" * 60)
    print("Module 05 — Workflow B: scANVI Integration")
    ...
    for obj_name, obj_metrics in all_metrics.items():
        for tier, tier_metrics in obj_metrics.items():
            if isinstance(tier_metrics, dict):
                row = {'object': obj_name, 'tier': tier, 'workflow': 'scanvi'}
